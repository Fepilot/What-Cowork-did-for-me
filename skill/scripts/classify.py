...
def load_taxonomy() -> tuple:
    with open(_TAXONOMY_PATH) as f:
        entries = json.load(f)
    pillar_lookup = {
        e["label"]: (e.get("value_pillar", "Improved Performance"),
                     e.get("pillar_css", "perf"))
        for e in entries
    }
    # Returns ((vectors, idf), pillar_lookup)
    return build_index(entries), pillar_lookup
...
def main(inp: str, out: str) -> None:
    ...
    (vectors, idf), pillar_lookup = load_taxonomy()
    ...
    for s in sessions:
        ...
        if sid in overrides:
            process = overrides[sid]
        elif primary == "general":
            process = "General Productivity"
        else:
            process = nearest(s.get("goal", ""), primary, vectors, idf)

        # value_pillar from taxonomy — data-driven, no hardcoded map in build_report
        value_pillar, pillar_css = pillar_lookup.get(process, ("Improved Performance", "perf"))
        ...
        if s.get("tool_calls") is not None:
            rec["tool_calls"] = s["tool_calls"]
